...
TEST(ConnectionTest, ComplexConnectionModuleRouting) {
    ...
    for (auto route : conn1->getRouteList()) {
        std::cout << "Gate: " << route.gate->getHost()->getPosition().x << " " << route.gate->getHost()->getPosition().y << std::endl;
        std::cout << "Destination: " << route.destination->getHost()->getPosition().x << " " << route.destination->getHost()->getPosition().y << std::endl;
    }

    std::cout << "Connection 2" << std::endl;
    for (auto route : conn2->getRouteList()) {
        std::cout << "Gate: " << route.gate->getHost()->getPosition().x << " " << route.gate->getHost()->getPosition().y << std::endl;
        std::cout << "Destination: " << route.destination->getHost()->getPosition().x << " " << route.destination->getHost()->getPosition().y << std::endl;
    }

    std::cout << "Connection 3" << std::endl;
    for (auto route : conn3->getRouteList()) {
        std::cout << "Gate: " << route.gate->getHost()->getPosition().x << " " << route.gate->getHost()->getPosition().y << std::endl;
        std::cout << "Destination: " << route.destination->getHost()->getPosition().x << " " << route.destination->getHost()->getPosition().y << std::endl;
    }*/
}